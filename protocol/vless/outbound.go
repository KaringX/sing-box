...
type Outbound struct {
	...
	packetAddr      bool
	xudp            bool
	encryption      *encryption.ClientInstance // https://github.com/starifly/sing-box
	vision          bool                       // https://github.com/starifly/sing-box
}
...
func NewOutbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VLESSOutboundOptions) (adapter.Outbound, error) {
	...
	outbound := &Outbound{
		...
		vision:     strings.HasPrefix(options.Flow, "xtls-rprx-vision"), // https://github.com/starifly/sing-box
	}
	if options.TLS != nil {
	...
	if options.Transport != nil {
		outbound.transport, err = v2ray.NewClientTransport(ctx, logger, outbound.dialer, outbound.serverAddr, common.PtrValueOrDefault(options.Transport), outbound.tlsConfig) //karing
		if err != nil {
...
func (h *vlessDialer) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	...
	if h.transport != nil {
		...
		if err == nil && h.tlsConfig != nil { // https://github.com/starifly/sing-box
	...
	case N.NetworkTCP:
		h.logger.InfoContext(ctx, "outbound connection to ", destination)
		if h.vision && visionBaseConn != nil { // https://github.com/starifly/sing-box
