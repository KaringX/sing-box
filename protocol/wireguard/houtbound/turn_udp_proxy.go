...
func ApplyTurnRelay(option CommonTurnRelayOptions) (*Forwarder, error) { //karing
	...
	if err != nil {
		//log.Panicf("Failed to connect to TURN server: %s", err)//karing
		return nil, E.Cause(err, "Failed to connect to TURN server") //karing
	}
	...
	if err != nil {
		//log.Panicf("Failed to create TURN client: %s", err)//karing
		return nil, E.Cause(err, "Failed to create to TURN server") //karing
	}
	defer client.Close()
	...
	err = client.Listen()
	if err != nil {
		//log.Panicf("Failed to listen: %s", err)//karing
		return nil, E.Cause(err, "Failed to listen") //karing
	}
	...
	if err != nil {
		//log.Panicf("Failed to allocate: %s", err)//karing
		return nil, E.Cause(err, "Failed to allocate") //karing
	}
	defer func() {
	...
	if err != nil {
		//log.Panicf("Failed to get random port: %s", err)//karing
		return nil, E.Cause(err, "Failed to get random port") //karing
	}
